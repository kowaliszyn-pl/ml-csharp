...
public static class OperationBackend
{
    ...
    internal static float SoftmaxCrossEntropyLoss(float[,] predicted, float[,] target, out float[,] softmaxOutput, float eps = 1e-7f)
    ...
    internal static float BinaryCrossEntropyLoss(float[,] predicted, float[,] target, float eps = 1e-7f)
       => Current.SigmoidBinaryCrossEntropyLoss(predicted, target, eps);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static float[,] BinaryCrossEntropyLossGradient(float[,] predicted, float[,] target)
        => Current.SigmoidBinaryCrossEntropyLossGradient(predicted, target);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static float[,,,] BiasAddConv2DOutput(float[,,,] input, float[] bias)
        => Current.BiasAddConv2DOutput(input, bias);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static float[] BiasAddConv2DParamGradient(float[,,,] outputGradient)
        // outputGradient is already averaged over the batch size in the loss function, so we just need to sum by columns
        => Current.BiasAddConv2DParamGradient(outputGradient);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static float[,,,] Unflatten(float[,] source, int dim2, int dim3, int dim4)
        => Current.Unflatten(source, dim2, dim3, dim4);
