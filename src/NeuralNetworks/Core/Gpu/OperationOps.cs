...
public static class OperationOps
{
    ...
    public static float[,] WeightMultiplyCalcOutput(float[,] input, float[,] weights)
    {
        ...
        using Context context = Context.Create(builder => builder.Cuda().CPU());
        using Accelerator accelerator = context.GetPreferredDevice(preferCPU: false).CreateAccelerator(context);

        using MemoryBuffer2D<float, Stride2D.DenseX> inputDev = accelerator.Allocate2DDenseX<float>(new Index2D(batchSize, inputFeatures));
        using MemoryBuffer2D<float, Stride2D.DenseX> weightsDev = accelerator.Allocate2DDenseX<float>(new Index2D(inputFeatures, outputFeatures));
        using MemoryBuffer2D<float, Stride2D.DenseX> outputDev = accelerator.Allocate2DDenseX<float>(new Index2D(batchSize, outputFeatures));
        ...
        Action<Index2D, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, int> kernel =
            accelerator.LoadAutoGroupedStreamKernel<Index2D, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, int>(WeightMultiplyOutputKernel);
    ...
    public static float[,] WeightMultiplyCalcInputGradient(float[,] outputGradient, float[,] weights)
    {
        ...
        using MemoryBuffer2D<float, Stride2D.DenseX> outputGradientDev = accelerator.Allocate2DDenseX<float>(new Index2D(batchSize, outputFeatures));
        using MemoryBuffer2D<float, Stride2D.DenseX> weightsDev = accelerator.Allocate2DDenseX<float>(new Index2D(inputFeatures, outputFeatures));
        using MemoryBuffer2D<float, Stride2D.DenseX> inputGradientDev = accelerator.Allocate2DDenseX<float>(new Index2D(batchSize, inputFeatures));

        outputGradientDev.View.CopyFromCPU(outputGradient);
        weightsDev.View.CopyFromCPU(weights);

        Action<Index2D, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, int> kernel =
            accelerator.LoadAutoGroupedStreamKernel<Index2D, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, int>(WeightMultiplyInputGradientKernel);
    ...
    public static float[,] WeightMultiplyCalcParamGradient(float[,] input, float[,] outputGradient)
    {
        ...
        using MemoryBuffer2D<float, Stride2D.DenseX> inputDev = accelerator.Allocate2DDenseX<float>(new Index2D(batchSize, inputFeatures));
        using MemoryBuffer2D<float, Stride2D.DenseX> outputGradientDev = accelerator.Allocate2DDenseX<float>(new Index2D(batchSize, outputFeatures));
        using MemoryBuffer2D<float, Stride2D.DenseX> paramGradientDev = accelerator.Allocate2DDenseX<float>(new Index2D(inputFeatures, outputFeatures));
        ...
        Action<Index2D, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>> kernel =
            accelerator.LoadAutoGroupedStreamKernel<Index2D, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>, ArrayView2D<float, Stride2D.DenseX>>(WeightMultiplyParamGradientKernel);
