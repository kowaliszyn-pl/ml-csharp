...
/// <summary>
/// Represents a one-dimensional convolutional layer that applies multiple filters to input data, enabling feature
...
/// language processing. The combination of filters, kernel size, stride, and dilation allows for flexible feature
/// extraction from one-dimensional data.
/// <para/>
/// The input data is [batch, channels, width].
/// The output data is [batch, filters, width].
/// </remarks>
/// <param name="filters">The number of convolution filters to apply. Determines the depth of the output feature map. Must be a positive
/// integer.</param>
/// <param name="kernelSize">The size of the convolution kernel. Specifies the width of each filter applied to the input data. Must be a positive
/// integer.</param>
...
public class Conv1DLayer(int filters, int kernelSize, ActivationFunction2D activationFunction, ParamInitializer paramInitializer, Dropout2D? dropout = null, bool addBias = false, int? padding = null, int stride = 1, int dilatation = 0) : Layer<float[,,], float[,,]>
{
    ...
    protected override void EnsureSameShapeForInput(float[,,]? input, float[,,]? inputGradient) => throw new NotImplementedException();
